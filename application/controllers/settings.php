...
//		$config['height']	= 320;
	...
	public function photo(){
		...
		if($_POST){
			...
			if(isset($_FILES['up_file']['name'])){
				$path = 'img/fans/'.$log_user_id;
				 
				if (!is_dir($path))
					mkdir($path, 0755);
  
				$pathMain = './img/fans/'.$log_user_id;
				if (!is_dir($pathMain))
					mkdir($pathMain, 0755);
			...
			if($save_path=='' && $save_path400=='' && $save_path100==''){
				...
				if($this->users->update_user($log_user_id, $upd_data) > 0){
					//add data to session
					$s_data = array (
						'sfh_user_pics' => $save_path400,
						'sfh_user_pics_small' => $save_path100
					);
		...
		$data['title'] = 'Profile Photo | SoccerFanHub';
	...
	public function account(){
			...
			if($_POST) {
				...
				if($log_user_email != $email) {
					if(($this->users->check_by_email($email) > 0) || ($this->users->check_by_username($nicename) > 0)) {
						$data['err_msg'] = '<h5 class="alert alert-info">Username or Email address already registered, try another</h5>';
					}
				} else {
					$update_data = array(
						'user_login' => $nicename,
					...
					if($this->users->update_user($log_user_id, $update_data) > 0){
						$data['err_msg'] = '<h5 class="alert alert-info">Account update successful</h5>';
						
						//update session records
						$session_data = array (
							'sfh_user_nicename' => $nicename,
							'sfh_user_email' => $email,
							'sfh_display_name' => $displayname,
							'sfh_user_bio' => $bio,
							'sfh_user_dob' => $dob,
							'sfh_user_sex' => $sex,
							'sfh_user_city' => $city,
							'sfh_user_phone' => $phone,
							'sfh_user_address' => $address,
							'sfh_user_website' => $website,
							'sfh_user_facebook' => $fb,
							'sfh_user_twitter' => $twitter,
							'sfh_user_linkedin' => $linkedin
						);
		...
		$data['title'] = 'Update Profile | SoccerFanHub';
		$data['page_active'] = 'setting';
	...
	public function password(){
		...
		$data['title'] = 'Change Password | SoccerFanHub';
		$data['page_active'] = 'setting';
